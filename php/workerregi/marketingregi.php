<html>
    ...
    <body>
        ...
        <div class="site2">
            <h1>Register as Worker for Marketing</h1>
            <form action="marketingregi.php" method="post">
                <form action="">
...
if (isset($_POST['vorname']) && isset($_POST['nachname']) && isset($_POST['email']) && isset($_POST['password']) && isset($_POST['service']) && isset($_POST['preis'])) {
    ...
    if ($count > 0) {
        ...
        // Überprüfen, ob die E-Mail-Adresse bereits in der 'marketingworker'-Tabelle vorhanden ist
        $query_check_marketingworker = "SELECT COUNT(*) AS count FROM `marketingworker` WHERE `Email` = ?";
        $stmt_check_marketingworker = mysqli_prepare($db, $query_check_marketingworker);
        mysqli_stmt_bind_param($stmt_check_marketingworker, "s", $email);
        mysqli_stmt_execute($stmt_check_marketingworker);
        mysqli_stmt_bind_result($stmt_check_marketingworker, $count_marketingworker);
        mysqli_stmt_fetch($stmt_check_marketingworker);
        mysqli_stmt_close($stmt_check_marketingworker);

        if ($count_marketingworker > 0) {
            // Die E-Mail-Adresse existiert bereits in der 'marketingworker'-Tabelle
            $_SESSION['username'] = $vorname; // Speichern des Benutzernamens in der Session
            echo "<script>alert('Email bereits vergeben'); location.href='marketingregi.php';</script>";
        } else {
            // Die E-Mail-Adresse existiert noch nicht in der 'marketingworker'-Tabelle
            // Füge den Eintrag in die 'marketingworker'-Tabelle ein
            $query_marketingworker = "INSERT INTO `marketingworker`(`Vorname`, `Nachname`, `Email`, `Passwort`, `Preis`, `Service`) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt_marketingworker = mysqli_prepare($db, $query_marketingworker);
            mysqli_stmt_bind_param($stmt_marketingworker, "ssssss", $vorname, $nachname, $email, $password, $preis, $service);
            $result_marketingworker = mysqli_stmt_execute($stmt_marketingworker);

            if ($result_marketingworker) {
                ...
                if ($result_benutzer) {
                    ...
                    echo "<script>alert('Fehler beim Erstellen des Eintrags für Benutzer.'); location.href='marketingregi.php';</script>";
                }
            } else {
                echo "<script>alert('Fehler beim Erstellen des Eintrags für Marketing-Worker.'); location.href='marketingregi.php';</script>";
            }
        }
    } else {
        ...
        // Eintrag in die 'marketingworker'-Tabelle einfügen
        $query_marketingworker = "INSERT INTO `marketingworker`(`Vorname`, `Nachname`, `Email`, `Passwort`, `Preis`, `Service`) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt_marketingworker = mysqli_prepare($db, $query_marketingworker);
        mysqli_stmt_bind_param($stmt_marketingworker, "ssssss", $vorname, $nachname, $email, $password, $preis, $service);
        $result_marketingworker = mysqli_stmt_execute($stmt_marketingworker);

        if ($result_benutzer && $result_marketingworker) {
            $_SESSION['username'] = $vorname; // Speichern des Benutzernamens in der Session
            echo "<script>alert('Registrierung erfolgreich!'); location.href='../index.php';</script>";
        } else {
            echo "<script>alert('Fehler beim Erstellen des Eintrags für Benutzer oder Marketing-Worker.'); location.href='marketingregi.php';</script>";
        }
    }
}
?>
